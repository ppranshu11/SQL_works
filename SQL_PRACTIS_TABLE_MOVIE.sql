...
SELECT DISTINCT MOVIE_SHOOTING_CITY FROM MOVIE;
SELECT SUBSTRING(ACTOR_NAME,1,3) FROM MOVIE;
SELECT INSTR (ACTOR_NAME,BINARY'A') FROM MOVIE WHERE ACTOR_NAME ="GOVINDA";
SELECT MIN(MOVIE_COST),MOVIE_SHOOTING_CITY FROM MOVIE GROUP BY MOVIE_SHOOTING_CITY;
/*COPYING DATA FROM ANOTHER DATABASE TO EMPTY DATABES*/
CREATE TABLE MY_PROJECT AS SELECT ACTOR_NAME,ACTRESS_NAME FROM MOVIE WHERE 1=2;
DROP TABLE MY_PROJECT;
INSERT INTO MY_PROJECT SELECT ACTOR_NAME,ACTRESS_NAME FROM MOVIE;
SELECT*FROM MY_PROJECT;
SELECT COUNT(ACTOR_NAME) FROM MOVIE;
